...
var jetson = function(){
	...
	socket.on("connect",()=>{
		...
		let idfriend=$("#idfriend").val();
	//	alert(idfriend)
		
		socket.emit("check-room",idfriend)
	})
	//alert(idfriend)
	...
	socket.on("send-mess-to-friend",(data)=>{
		console.log(data)
		$("#chat_message").append(`<li class="friend">
	...
	socket.on("load-data-from-database",(dataMess)=>{
		   console.log(dataMess.data)
		   if(dataMess.data.length>0){
			   dataMess.data.forEach(element => {
				   if(element.user_id==dataMess.user_id){
					   $("#chat_message").append(`<li class="self">
					   <div class="self-msg-wrap">
						   <div class="msg block pull-right">`+ element.message +
							   `<div class="msg-per-detail text-right">
					   ...
					   $("#chat_message").append(`<li class="friend">
					   <div class="friend-msg-wrap">
						   <img class="user-img img-circle block pull-left"  src="../dist/img/profile/`+element.avatar+`" alt="user"/>
						   <div class="msg pull-left">
							   <p>`+element.message+`</p>
							   <div class="msg-per-detail text-right">
								   <span class="msg-time txt-grey">2:30 PM</span>
